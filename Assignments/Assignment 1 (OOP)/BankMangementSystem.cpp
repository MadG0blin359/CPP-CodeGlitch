...
void depositFunds(int, long[], double[], long , int);       // Deposit Function Overload
void withdrawFunds(int, long[], double[]);
void withdrawFunds(int, long[], double[], long, int);      // Withdraw Function Overload
...
void depositFunds(int numOfAccounts, long accountNumbers[], double accountBalances[]) {
    ...
    for (int i = 0; i <= numOfAccounts; i++) {
        if (accountNumbers[i] == account_number) {      // Condition, if the account number is found
            ...
            int integer_value = deposit_amount;     // Converting user input into an integer value

            if (deposit_amount/integer_value == 1) {
                // Overload function calling if the user inputs an integer value
                depositFunds(numOfAccounts, accountNumbers, accountBalances, account_number, deposit_amount);
            }
...
// Overload Function of depositFunds
void depositFunds(int numOfAccounts, long accountNumbers[], double accountBalances[], long account_number, int deposit_amount) {
    for (int i = 0; i <= numOfAccounts; i++) {
        if (accountNumbers[i] == account_number) {
            accountBalances[i] += deposit_amount;       // Update the current account balance
...
void withdrawFunds(int numOfAccounts, long accountNumbers[], double accountBalances[]) {
    ...
    for (int i = 0; i <= numOfAccounts; i++) {
        if (accountNumbers[i] == account_number) {      // Condition, if the account number is found
            ...
            int integer_value = withdraw_amount;     // Converting user input into an integer value

            if (withdraw_amount/integer_value == 1) {
                // Overload function calling if the user inputs an integer value
                withdrawFunds(numOfAccounts, accountNumbers, accountBalances, account_number, withdraw_amount);
            }
...
// Overload Function of withdrawFunds
void withdrawFunds(int numOfAccounts, long accountNumbers[], double accountBalances[], long account_number, int withdraw_amount) {
    for (int i = 0; i <= numOfAccounts; i++) {
        if (accountNumbers[i] == account_number) {
            accountBalances[i] -= withdraw_amount;       // Update the current account balance
